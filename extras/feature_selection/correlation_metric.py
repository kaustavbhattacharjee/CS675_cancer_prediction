...
class CorrelationMetric(object):
    ...
    def get_correlation(self, dataset, labels): pass
...
class PearsonCorrelation(CorrelationMetric):
    ...
    def get_correlation(self, dataset, labels):
        return map(lambda feature: self._feature_correlation(feature, labels), zip(*dataset))
...
class ChiSquare(CorrelationMetric):
    def __init__(self):
    ...
    def get_correlation(self, dataset, labels):
        l = len(dataset)
        Y = [[1 - label for label in labels]]
        Y.append(labels)

        observed = LinAlg.dot_product(Y, dataset)
        means = [[sum(yi) / l] for yi in Y]
        sums = [[sum(feature) for feature in zip(*dataset)]]
        estimation = LinAlg.dot_product(means, sums)

        chisq = [[((i - j) ** 2) / j for i, j in zip(oi, ei)] for oi, ei in zip(observed, estimation)]

        return map(self._feature_correlation, zip(*chisq))